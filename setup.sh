...
# Tento skript musí být spuštěn s právy sudo nebo uživatelem, který má sudo práva.
set -e

echo "=== Spouštím instalaci KvízAréna Serveru ==="

# Krok 1: Aktualizace systému a instalace základních balíčků
echo "[1/4] Aktualizace balíčků a instalace python3-venv a python3-pip..."
sudo apt-get update
...
# Musíme aktivovat v subshellu, aby `pip` fungoval
...
if [ ! -f "$ENV_FILE" ]; then
    ...
    # Použijeme sed s oddělovačem #, aby fungoval i s divnými znaky
    sed -i "s#SECRET_KEY=change-me#SECRET_KEY=$NEW_KEY#" .env
    echo "DŮLEŽITÉ: .env soubor byl vytvořen. Zkontrolujte jeho obsah (hlavně DATABASE_URL)."
else
...
echo "  flask init-db  <-- POZOR: Tento příkaz musíme teprve naprogramovat!"
echo ""
